...
def resolver_combate(cant_atacante, cant_defensor, provincia,
    ...
    """Seccion 2 completa. Combate ronda a ronda (Ley Cuadratica de Lanchester):
      - PE_atacante = A(t) * alpha_ataque * theta_terreno,atacante * X_A        (2.1)
      - PE_defensor = D(t) * alpha_defensa * theta_terreno,defensor * X_D       (2.1)
      - A(tau+1) = max(0, A(tau) - beta * PE_defensor)                         (2.4)
      - D(tau+1) = max(0, D(tau) - alpha * PE_atacante * (1 - phi_fort * Fort_i)) (2.4)
    La fortificacion (phi_fort = 50%) reduce las bajas del defensor a la mitad.
    Se detiene cuando un bando es aniquilado o al llegar a MAX_RONDAS_COMBATE.
    Devuelve el resultado segun la Seccion 2.5 y el registro de cada ronda."""
